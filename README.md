...
- 视频一律走分片上传：腾讯的富媒体接口已经不接受 base64 直传的视频（返回“上传URL错误”），而 AstrBot 适配器只对 10 MB 以上的文件用分片上传，插件在官 Bot 模式下会把这个阈值调成 0，让所有本地视频 / 文件都走文档里的分片上传接口。图片不受影响。
- 一条结果里的多个媒体会按“一条消息一个媒体”分开发送，某一项上传失败不会连累其它项。
...
官 Bot（QQ 官方机器人）收到的小程序卡片没有链接，平台只给一段 `[卡片消息] 小程序 / 摘要 / preview / source: 哔哩哔哩 / title: ...` 的文本。插件会拿 `title` 去 B站搜同名视频（优先完全同名，其次标题被截断时按前缀匹配），搜到就按普通视频解析；搜不到会提示“B站没有搜到同名视频”。

搜索走的是和评论区一样的带浏览器指纹、带登录 Cookie 的会话：普通搜索接口和 WBI 搜索接口都会问一遍（两边索引不完全一样），访客态偶尔会被 B站风控打回只带 `v_voucher` 的空响应，插件会隔一会儿重试，最多三轮，一直被拦会提示“搜索接口被风控”。配置了 B站 Cookie 或扫码登录后基本不会被拦。
